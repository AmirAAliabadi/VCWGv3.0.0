"""
Main VCWG script
Developed by Mojtaba Safdari, Mohsen Moradi and Amir A. Aliabadi
...
class VCWG_Smart(object):

    def __init__(self,epwFileName,TopForcingFileName,VCWGParamFileName,ViewFactorFileName,case,month_name, date_str):
    ...
    def read_input(self):
        ...
        self.charLength,self.BEMParam,self.SmartBuildingParam, self.TOU = Data_Site(self.VCWGParamFileName)
    ...
    def Simulate(self):
        ...
        self.SmartThermostat = [None for x in range(self.N)]
        self.SmartHumidistat = [None for x in range(self.N)]
        ...
        for it in range(0,self.simTime.nt-1,1):
            progress_percentage = numpy.round(100 * it / self.simTime.nt, 2)
            # Print progress every 10 iterations
            if it % 10 == 0:
                print(f"Progress [{self.month_name}]: {progress_percentage}%")
            ...
            for i in range(len(self.BEM)):
                ...
                self.BEM[i].building.coolTempSetpointDay = self.Sch[i].Cool[self.dayType - 1][self.simTime.hourDay] + 273.15
                self.BEM[i].building.coolTempSetpointNight = self.BEM[i].building.coolTempSetpointDay
                # Add from temperature schedule for heating
                self.BEM[i].building.heatTempSetpointDay = self.Sch[i].Heat[self.dayType - 1][self.simTime.hourDay] + 273.15
                self.BEM[i].building.heatTempSetpointNight = self.BEM[i].building.heatTempSetpointDay
                ...
                # Add from Occupant Presence Schedule
                self.BEM[i].building.Nocc = self.Sch[i].Nocc * self.Sch[i].Occ[self.dayType - 1][self.simTime.hourDay]
                ...
                self.BEM[i].building.BEMCalc(canTemp,canHum,self.BEM[i],MeteoData,ParCalculation,self.simTime,self.Geometry_m,
                                             self.FractionsRoof,self.EBCanyon.SWR,self.SmartBuildingParam, self.TOU)
    ...
    def write_output(self):
        ...
        # Create folder with current date and hour within Results directory
        Output_dir = os.path.join("Results", self.date_str)
        os.makedirs(Output_dir, exist_ok=True)  # Create folder if it doesn't exist, otherwise ignore

        Write_Forcing(self.case,self.ForcingData,self.time,Output_dir, self.month_name)

        Write_EB(self.case,self.FractionsRoof, self.FractionsGround, self.ParTree, self.RSMParam, self.EBRoofData,self.EBCanyonData,self.EBRuralData,self.UCMData,self.time,Output_dir, self.month_name)
        ...
        Write_Tsurf(self.case, self.FractionsRoof, self.FractionsGround, self.ParTree,self.RoofImpData,self.RoofVegData,self.GroundImpData,self.GroundBareData,self.GroundVegData,self.WallSunData,
                    ...
                    self.RSMParam,self.time,Output_dir, self.month_name)

        Write_WB(self.case,self.FractionsRoof, self.FractionsGround, self.ParTree,self.WBRoofData,self.WBCanyonData,self.time, Output_dir, self.month_name)

        Write_BEM(self.BEMData,self.time,self.case,Output_dir, self.month_name)
        ...
        Write_TdeepProfiles("Tdeep_imp",self.FractionsGround,1,self.GroundImpData,self.GroundImpData[0].z_depth[1:],self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for deep vegetated ground temperature profile
        Write_TdeepProfiles("Tdeep_veg",self.FractionsGround,2,self.GroundVegData,self.GroundVegData[0].z_depth[1:],self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for deep bare ground temperature profile
        Write_TdeepProfiles("Tdeep_bare",self.FractionsGround,3,self.GroundBareData,self.GroundBareData[0].z_depth[1:],self.time,self.case,Output_dir, self.month_name)

        # Generate output text file for Qn profile
        Write_1Dprofiles("Qn",self.UCMData,'VerticalProfUrban','qn',self.Geometry_m.z,self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for tke profile
        Write_1Dprofiles("TKE",self.UCMData,'VerticalProfUrban','tke',self.Geometry_m.z,self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for th profile
        Write_1Dprofiles("th",self.UCMData,'VerticalProfUrban','th',self.Geometry_m.z,self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for vx profile
        Write_1Dprofiles("vx",self.UCMData,'VerticalProfUrban','vx',self.Geometry_m.z,self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for vy profile
        Write_1Dprofiles("vy",self.UCMData,'VerticalProfUrban','vy',self.Geometry_m.z,self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for S profile
        Write_1Dprofiles("S",self.UCMData,'VerticalProfUrban','s',self.Geometry_m.z,self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for LE profile
        Write_1Dprofiles("LEfluxProf", self.UCMData, 'VerticalProfUrban', 'LEflux', self.Geometry_m.z, self.time, self.case,Output_dir, self.month_name)
        # Generate output text file for H profile
        Write_1Dprofiles("HfluxProf", self.UCMData, 'VerticalProfUrban', 'Hflux', self.Geometry_m.z, self.time, self.case,Output_dir, self.month_name)

        # Generate output text file for th profile in rural area
        Write_Ruralprofiles("th_rural",self.RSMParam.Rural_Model_name,self.RSMData,'T_rural',self.RSMData[0].z,self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for q profile in rural area
        Write_Ruralprofiles("Qn_rural",self.RSMParam.Rural_Model_name,self.RSMData,'q_rural',self.RSMData[0].z,self.time,self.case,Output_dir, self.month_name)
        # Generate output text file for P profile in rural area
        Write_Ruralprofiles("P_rural",self.RSMParam.Rural_Model_name,self.RSMData,'presProf',self.RSMData[0].z,self.time,self.case,Output_dir, self.month_name)
