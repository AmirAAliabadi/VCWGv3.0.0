...
def read_VCWG_param(VCWG_param_file_path):
    ...
    while count < len(VCWG_param):
        ...
        if row == [] or "#" in row[0]:
            ...
            count += 2
        elif row[0] == "TOU_Low_Summer":
            # TOU_Low_Summer
            TOU_Low_Summer_Rows = VCWG_param[count + 1:count + 2]
            TOU_Low_Summer_Rrows_float = []
            for i in range(len(TOU_Low_Summer_Rows)):
                TOU_Low_Summer_Rrows_float.append(list(map(float, TOU_Low_Summer_Rows[i][:-1])))
            _init_param_dict[row[0]] = TOU_Low_Summer_Rrows_float
            count += 2
        
        elif row[0] == "TOU_Medium_Summer":
            # TOU_Medium_Summer
            TOU_Medium_Summer_Rows = VCWG_param[count + 1:count + 2]
            TOU_Medium_Summer_Rrows_float = []
            for i in range(len(TOU_Medium_Summer_Rows)):
                TOU_Medium_Summer_Rrows_float.append(list(map(float, TOU_Medium_Summer_Rows[i][:-1])))
            _init_param_dict[row[0]] = TOU_Medium_Summer_Rrows_float
            count += 2
        
        elif row[0] == "TOU_High_Summer":
            # TOU_High_Summer
            TOU_High_Summer_Rows = VCWG_param[count + 1:count + 2]
            TOU_High_Summer_Rrows_float = []
            for i in range(len(TOU_High_Summer_Rows)):
                TOU_High_Summer_Rrows_float.append(list(map(float, TOU_High_Summer_Rows[i][:-1])))
            _init_param_dict[row[0]] = TOU_High_Summer_Rrows_float
            count += 2
        
        elif row[0] == "TOU_Low_Winter":
            # TOU_Low_Winter
            TOU_Low_Winter_Rows = VCWG_param[count + 1:count + 2]
            TOU_Low_Winter_Rrows_float = []
            for i in range(len(TOU_Low_Winter_Rows)):
                TOU_Low_Winter_Rrows_float.append(list(map(float, TOU_Low_Winter_Rows[i][:-1])))
            _init_param_dict[row[0]] = TOU_Low_Winter_Rrows_float
            count += 2
        
        elif row[0] == "TOU_Medium_Winter":
            # TOU_Medium_Winter
            TOU_Medium_Winter_Rows = VCWG_param[count + 1:count + 2]
            TOU_Medium_Winter_Rrows_float = []
            for i in range(len(TOU_Medium_Winter_Rows)):
                TOU_Medium_Winter_Rrows_float.append(list(map(float, TOU_Medium_Winter_Rows[i][:-1])))
            _init_param_dict[row[0]] = TOU_Medium_Winter_Rrows_float
            count += 2
        
        elif row[0] == "TOU_High_Winter":
            # TOU_High_Winter
            TOU_High_Winter_Rows = VCWG_param[count + 1:count + 2]
            TOU_High_Winter_Rrows_float = []
            for i in range(len(TOU_High_Winter_Rows)):
                TOU_High_Winter_Rrows_float.append(list(map(float, TOU_High_Winter_Rows[i][:-1])))
            _init_param_dict[row[0]] = TOU_High_Winter_Rrows_float
            count += 2
        else:
            if row[1] == 'NaN':
...
def Data_Site(InputFile):
    ...
    TOU_Low_Summer = ipd['TOU_Low_Summer'][0]
    TOU_Medium_Summer = ipd['TOU_Medium_Summer'][0]
    TOU_High_Summer = ipd['TOU_High_Summer'][0]
    TOU_Low_Winter = ipd['TOU_Low_Winter'][0]
    TOU_Medium_Winter = ipd['TOU_Medium_Winter'][0]
    TOU_High_Winter = ipd['TOU_High_Winter'][0]
    ...
    class SmartBuildingParamDef():
        pass
    SmartBuildingParam = SmartBuildingParamDef()
    SmartBuildingParam.SmartThermostat = ipd['SmartThermostat']
    SmartBuildingParam.SmartHumidistat = ipd['SmartHumidistat']

    SmartBuildingParam.coolTempSetpointDay = ipd['coolTempSetpointDay']
    SmartBuildingParam.coolTempSetpointNight = ipd['coolTempSetpointNight']
    SmartBuildingParam.heatTempSetpointDay = ipd['heatTempSetpointDay']
    SmartBuildingParam.heatTempSetpointNight = ipd['heatTempSetpointNight']
    SmartBuildingParam.dehumRHSetpointDay = ipd['dehumRHSetpointDay']
    SmartBuildingParam.dehumRHSetpointNight = ipd['dehumRHSetpointNight']
    SmartBuildingParam.humRHSetpointDay = ipd['humRHSetpointDay']
    SmartBuildingParam.humRHSetpointNight = ipd['humRHSetpointNight']

    SmartBuildingParam.heatTempSetpointHighHigh = ipd['heatTempSetpointHighHigh']
    SmartBuildingParam.heatTempSetpointHigh = ipd['heatTempSetpointHigh']
    SmartBuildingParam.heatTempSetpointLow = ipd['heatTempSetpointLow']
    SmartBuildingParam.heatTempSetpointLowLow = ipd['heatTempSetpointLowLow']

    SmartBuildingParam.coolTempSetpointHighHigh = ipd['coolTempSetpointHighHigh']
    SmartBuildingParam.coolTempSetpointHigh = ipd['coolTempSetpointHigh']
    SmartBuildingParam.coolTempSetpointLow = ipd['coolTempSetpointLow']
    SmartBuildingParam.coolTempSetpointLowLow = ipd['coolTempSetpointLowLow']

    SmartBuildingParam.RHSetPointLow = ipd['RHSetPointLow']
    SmartBuildingParam.RHSetPointHigh = ipd['RHSetPointHigh']
    SmartBuildingParam.RHSetPointLowLow = ipd['RHSetPointLowLow']
    SmartBuildingParam.RHSetPointHighHigh = ipd['RHSetPointHighHigh']

    SmartBuildingParam.Season = ipd['Season']
    
    class TOU():
        pass
    
    TOU.TOU_Low_Summer = TOU_Low_Summer
    TOU.TOU_Medium_Summer = TOU_Medium_Summer
    TOU.TOU_High_Summer = TOU_High_Summer
    TOU.TOU_Low_Winter = TOU_Low_Winter
    TOU.TOU_Medium_Winter = TOU_Medium_Winter
    TOU.TOU_High_Winter = TOU_High_Winter
    ...
    return Gemeotry_m,ParTree,geometry,FractionsRoof,FractionsGround,WallLayers,ParSoilRoof,ParSoilGround,ParInterceptionTree,\
           ...
           ParVegRoof,ParVegGround,ParVegTree,Person,ColParam,RSMParam,TimeParam,ViewFactorCal_Param,bld,zone,charLength,BEMParam,SmartBuildingParam, TOU