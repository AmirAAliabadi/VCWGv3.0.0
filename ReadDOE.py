...
def readDOE(serialize_output=True):
    ...
    #Nested loop = 16 types, 3 era, 17 zones = time complexity O(n*m*k) = 768
    ...
    for i in range(16):
        ...
        RvalWall    = df_LocationSummary.loc['RvalWall'].values.astype(float) # [m^2 K W^-1] R-value
        TypeRoof    = df_LocationSummary.loc['TypeRoof'].values                     # Construction type
        RvalRoof    = df_LocationSummary.loc['RvalRoof'].values.astype(float) # [m^2 K W^-1] R-value
        Uwindow     = df_LocationSummary.loc['Uwindow'].values.astype(float)  # [W m^-2 K^-1] U-factor
        SHGC        = df_LocationSummary.loc['SHGC'].values.astype(float)     # [-] coefficient
        HVAC        = df_LocationSummary.loc['HVAC'].values.astype(float)     # [kW] Air Conditioning
        HEAT        = df_LocationSummary.loc['HEAT'].values.astype(float)     # [kW] Heating
        COP         = df_LocationSummary.loc['COP'].values.astype(float)      # [-] Air Conditioning COP
        EffHeat     = df_LocationSummary.loc['EffHeat'].values.astype(float)  # [%] Heating Efficiency
        FanFlow     = df_LocationSummary.loc['Fan'].values.astype(float)      # [m^3 s^-1] Fan Max Flow Rate
        ...
        RHSetCool = df_Schedules.iloc[21:24].values    # Cooling RH Setpoint Schedule 24 hrs [%]
        RHSetHeat = df_Schedules.iloc[24:27].values     # Heating RH Setpoint Schedule 24 hrs; summer design [%]
        ...
        for j in range(3):
            ...
            for k in range(17):
                ...
                B = Building(
                    ...
                    293,                                # initial indoor Temp at 20 C
                    295,                 #initial Temp Set-Point [ C]
                    50)              #initial Rel Hum Set-Point [ %]
